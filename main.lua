Arrival = {}
Arrival.init = {}
Arrival.init.zonedata_full = {}
Arrival.init.currentzonedata = {}


Arrival.init.ped = nil
Arrival.init.pedcoords = vector3(0.0,0.0,0.0)
Arrival.init.pedzone = ''
Arrival.init.debuglog = true


Arrival.init.Register = function (datas,rangeorcb,_cb)
    ...
    local data = Arrival.init.ConvertData(datas)  -- to .x .y .z .index 
    local zonelist,zonedata = Arrival.init.CollectZoneData(data,range)
    for i,v in pairs (zonedata) do 
        local zone = v.zone
        v.arrival = fntotable(cb)
        v.range = range
        if not Arrival.init.zonedata_full[zone] then Arrival.init.zonedata_full[zone]={} end 
        table.insert(Arrival.init.zonedata_full[zone],v)
    end 
    
    if  GetCurrentResourceName() ~= resourceName or Arrival.init.debuglog then
        Threads.CreateLoopOnce('inits',528,function(delay)
            Arrival.init.ped = PlayerPedId()
            Arrival.init.pedcoords = GetEntityCoords(Arrival.init.ped)
            if Arrival.init.pedzone ~= Arrival.init.GetHashMethod(Arrival.init.pedcoords.x,Arrival.init.pedcoords.y,Arrival.init.pedcoords.z,range) then 
                local old = Arrival.init.pedzone
                if old and #old>0 then 
                    local zonedatas = Arrival.init.zonedata_full[old]
                    if zonedatas and #zonedatas>0 then 
                        for i=1,#zonedatas do 
                            local v = zonedatas[i]
                            local pos = vector3(v.x,v.y,v.z)
                            local distance = #(pos-Arrival.init.pedcoords)
            ...
            Arrival.init.pedzone = Arrival.init.GetHashMethod(Arrival.init.pedcoords.x,Arrival.init.pedcoords.y,Arrival.init.pedcoords.z,range)
            local zonedatas = Arrival.init.zonedata_full[Arrival.init.pedzone]
            if zonedatas and #zonedatas>0 then 
                for i=1,#zonedatas do 
                    local v = zonedatas[i]
                    local pos = vector3(v.x,v.y,v.z)
...
exports('Register', function(...)
  Arrival.init.Register(...)
end)


Arrival.init.getnearzones = function(...)
    local _pos = {...}
    if #{...} == 3 then 
    ...
    local pos = Arrival.init.pedcoords
    local temp_x = 0.0
...
Arrival.init.CollectZoneData = function(datatable,range) --vector3 or {x=1.0,y=2.0,z=3.0}
    ...
    for i=1,#datatable do 
        ...
        local zone = Arrival.init.GetHashMethod(v.x,v.y,v.z,range)
...
Arrival.init.ConvertData = function(datatable) 
    local tp = nil
    local result = {}
    ...
    if #datatable > 0 then
        if type(datatable) == 'table' then 
            ...
            error('Arrival.init.ConvertData(table)',2)
        end 
    else 
        error('data style not supported',2)
    end 
...
Arrival.init.GetHashMethod = function(x,y,z,range)
